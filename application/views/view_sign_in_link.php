<?php
// not used???
?><div class="main-content">

<p class="intro">Talk directly to God in 140 characters! Holy Tweet uses the miracle of our advanced ethereal net protocol to deliver your 140 character message directly to God.</p>
 

<a href="<?php echo site_url(); ?>sign_in" class="btn" id="sign-in-home">Sign in</a>

<p class="sign-up">Not a member?  <strong><a href="<?php echo site_url(); ?>join">Join now</a></strong></p>

</div><!-- .main-content -->

